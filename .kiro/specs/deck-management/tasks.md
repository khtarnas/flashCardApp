...
- [x] 1. Foundations: strip template, extend data model, add core types
  ...
  - [x] 1.4 Create `HanaHou/Models/Language.swift` with the `Language` enum (`String, CaseIterable, Codable, Hashable`) and cases from design §3
    - Pure data declaration — no tests required per TDD judgment (D007)
    - _Requirements: 2.2, 6.1_
  - [x] 1.5 Create `HanaHou/Models/DeckSnapshot.swift` and `HanaHou/Models/DeckDraft.swift` value types matching design §3
    - Pure value-type scaffolding — no tests required
    - _Requirements: 6.1_
  - [x] 1.6 Create `HanaHou/Models/DeckListItem.swift` with the `enum DeckListItem { case allCards; case deck(DeckSnapshot) }` and nested `ID` enum from design §4
    - Pure type declaration — no tests required; behavior is exercised via `DeckListComposer` tests
    - _Requirements: 1.3, 8.5_
  - [x] 1.7 Create `HanaHou/Models/DeckNameError.swift` with `empty`, `reserved(name:)`, `duplicate(name:)` cases per design §6
    - Pure type declaration — no tests required; behavior covered by validator tests
    - _Requirements: 5.5_
  - [x] 1.8 Create `HanaHou/Persistence/DeckStore.swift` defining the `DeckStore` protocol and `DeckStoreError` per design §7
    - Protocol declaration — no tests required; tested via concrete implementations
    - _Requirements: 6.1_
  - [x] 1.9 Create `HanaHou/Models/AllCardsActionError.swift` with `enum AllCardsActionError: Error { case notAllowed }` per design §9
    - _Requirements: 4.4, 8.3, 8.4_

- [x] 2. Domain services (tests first for each)
  - [x] 2.1 Write `HanaHouTests/Domain/DeckNameValidatorTests.swift`
  ...
  - [x] 2.2 Implement `HanaHou/Domain/DeckNameValidator.swift` to satisfy 2.1
  ...
  - [x] 2.3 Write `HanaHouTests/Domain/DeckOrderingStrategyTests.swift`
    - Covers B7 (creation-date ascending with `id.uuidString` tiebreaker on equal `createdAt`)
    - _Requirements: 1.6_
  - [x] 2.4 Implement `HanaHou/Domain/DeckOrderingStrategy.swift` with the protocol and `CreationDateAscendingOrdering` struct per design §5
    - _Requirements: 1.5, 1.6, 1.7_
  - [x] 2.5 Write `HanaHouTests/Domain/DeckListComposerTests.swift`
  ...
  - [x] 2.6 Implement `HanaHou/Domain/DeckListComposer.swift` producing `[.allCards] + strategy.order(userDecks).map(.deck)` per design §4
...
- [x] 4. Persistence layer
  - [x] 4.1 Write `HanaHouTests/Persistence/InMemoryDeckStoreTests.swift`
  ...
  - [x] 4.2 Implement `HanaHou/Persistence/InMemoryDeckStore.swift` per design §11 (test double usable as a real store for view-model tests)
  ...
  - [x] 4.3 Write `HanaHouTests/Persistence/CoreDataDeckStoreTests.swift`
  ...
  - [x] 4.4 Implement `HanaHou/Persistence/CoreDataDeckStore.swift` satisfying 4.3
  ...
  - [x] 4.5 Refactor `HanaHou/Persistence.swift`
...
- [x] 5. View models
  - [x] 5.1 Write `HanaHouTests/ViewModels/DeckEditorViewModelTests.swift`
  ...
  - [x] 5.2 Implement `HanaHou/ViewModels/DeckEditorViewModel.swift` satisfying 5.1
  ...
  - [x] 5.3 Write `HanaHouTests/ViewModels/DeckListViewModelTests.swift`
  ...
  - [x] 5.4 Implement `HanaHou/ViewModels/DeckListViewModel.swift` satisfying 5.3
...
- [x] 7. Views and navigation
  ...
  - [x] 7.1 Create `HanaHou/Views/DeckManagementRootView.swift` hosting the `NavigationStack` and owning `DeckListViewModel` as `@StateObject`
    - _Requirements: 7.1_
  - [x] 7.2 Create `HanaHou/Views/DeckListView.swift`
  ...
  - [x] 7.3 Create `HanaHou/Views/DeckEditorView.swift`
  ...
  - [x] 7.4 Create `HanaHou/Views/DeleteConfirmationDialog.swift` (or inline `.confirmationDialog` modifier) with copy "Delete \"\(name)\"? Cards in this deck will be kept and remain visible in All Cards."
    - _Requirements: 4.1, 4.2, 4.3_
  - [x] 7.5 Create `HanaHou/Views/AllCardsPlaceholderView.swift` with copy "All Cards view — coming in the card-management spec."
    - _Requirements: 8.2_
  - [x] 7.6 Wire view models to views in `DeckManagementRootView`: inject `DeckStore` and `DeckOrderingStrategy`, construct `DeckListViewModel`, route `DeckEditorView` via `NavigationStack` path
    - _Requirements: 1.7, 1.8, 7.1_
  - [x] 7.7 Write `HanaHouTests/Views/DeckManagementSmokeTests.swift` — example-based smoke tests for non-algorithmic items
...
- [x] 8. Orientation lock
  - [x] 8.1 Write `HanaHouTests/Configuration/InfoPlistOrientationTests.swift`
  ...
  - [x] 8.2 Set `UISupportedInterfaceOrientations~ipad` in the app target's Info.plist to `["UIInterfaceOrientationPortrait", "UIInterfaceOrientationPortraitUpsideDown"]`
    - Single point of change per D028 / design §8; no per-view overrides
    - _Requirements: 7.2, 7.3, 7.4, 7.5_

- [x] 9. Wire-up and documentation
  - [x] 9.1 Update `HanaHou/HanaHouApp.swift` to host `DeckManagementRootView`
  ...
  - [x] 9.2 Delete `HanaHou/ContentView.swift` once `DeckManagementRootView` is wired
    - Blocked by: 9.1
    - _Requirements: (cleanup)_
  - [x] 9.3 Add/update `AGENTS.md` in each new directory per D005
  ...
  - [x] 9.4 Append three new decisions to `docs/decisions.md` per design §12 "Open Design Decisions"
  ...
  - [x] 9.5 Update `docs/p0.md`: replace the "All Cards is a system concept, not a regular Deck entity (implementation TBD in mini-design)" line with a reference to the resolved design decision (D026 / design §4)
    - _Requirements: (docs alignment)_

- [x] 10. Final checkpoint — build and run the full test suite
